...
import { PRODUCT_DESTINATION } from './imageDestination.config';
import { v4 as uuid } from 'uuid';

export const multerOptionsProduct: MulterOptions = {
  limits: {
  ...
  storage: diskStorage({
    destination(
      ...
      if (!existsSync(PRODUCT_DESTINATION)) {
        mkdirSync(PRODUCT_DESTINATION);
      }
      done(null, PRODUCT_DESTINATION);
    },
    filename(
      req: Request,
      file: Express.Multer.File,
